  ...
  <d2-container  :class="{'page-compact':crud.pageOptions.compact}">
    <d2-crud-x
      ...
      <div slot="header">
        ...
        <el-button-group>
          <el-button size="small" type="primary" @click="addRow"><i class="el-icon-plus"/> 新增单位</el-button>
...
<script>
import { goodsUnitList, goodsUnitStore, goodsUnitUpdate, goodsUnitDestroy } from './api'
...
export default {
  ...
  methods: {
    ...
    pageRequest (query) {
      ...
      return goodsUnitList(query)
    },
    addRequest (row) {
      return goodsUnitStore(row)
    },
    updateRequest (row) {
      return goodsUnitUpdate(row)
    },
    delRequest (row) {
      return goodsUnitDestroy(row.id)
    },
    batchDelRequest (ids) {
      return goodsUnitDestroy(ids)
    }
  }
}
</script>
