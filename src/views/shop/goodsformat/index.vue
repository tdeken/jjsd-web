  ...
  <d2-container  :class="{'page-compact':crud.pageOptions.compact}">
    <d2-crud-x
      ...
      <div slot="header">
        ...
        <el-button-group>
          <el-button size="small" type="primary" @click="addRow"><i class="el-icon-plus"/> 新增规格</el-button>
...
<script>
import { goodsFormatList, goodsFormatStore, goodsFormatUpdate, goodsFormatDestroy } from './api'
...
export default {
  ...
  methods: {
    ...
    pageRequest (query) {
      ...
      return goodsFormatList(query)
    },
    addRequest (row) {
      return goodsFormatStore(row)
    },
    updateRequest (row) {
      return goodsFormatUpdate(row)
    },
    delRequest (row) {
      return goodsFormatDestroy(row.id)
    },
    batchDelRequest (ids) {
      return goodsFormatDestroy(ids)
    }
  }
}
</script>
