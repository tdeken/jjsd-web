<template>
  <d2-container style="background-color: #FFFFFF">
    ...
    <template slot="footer">
      <el-button slot="header" type="primary" @click="submitOrder" round style="float:right;margin-bottom: 5px">确认修改</el-button>
    </template>
    <el-dialog
      title="添加货物"
...
<script>
import { showRequest, bookList, bookGoods, bookClear, bookShow, bookUpdate, orderCreate } from '../store/api'
...
export default {
  ...
  mounted () {
    ...
    if (this.$route.query.order_id) {
      this.addGoodsForm.order_id = this.$route.query.order_id
    }
  ...
  methods: {
    ...
    submitOrder () {
      orderCreate({ address_id: this.addGoodsForm.address_id, order_id: this.addGoodsForm.order_id }).then(res => {
        this.$message({
    ...
    rowRemove ({ row }, done) {
      bookClear({ address_id: this.addGoodsForm.address_id, order_id: this.addGoodsForm.order_id, book_id: row.id }).then(res => {
        this.fetchData({ address_id: this.addGoodsForm.address_id, order_id: this.addGoodsForm.order_id })
        this.$message({
          message: '删除成功',
          type: 'success'
        })
    ...
    clearBookGoods () {
      this.$confirm('您确定清空货物吗？', '提示', {
        ...
        bookClear({ address_id: this.addGoodsForm.address_id, order_id: this.addGoodsForm.order_id }).then(res => {
          this.fetchData({ address_id: this.addGoodsForm.address_id, order_id: this.addGoodsForm.order_id })
          this.$message({
            message: '货物已清空',
            type: 'success'
